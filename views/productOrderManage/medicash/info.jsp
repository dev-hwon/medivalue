...
<%--		<script type="text/javascript" src="https://tbnpg.settlebank.co.kr/resources/js/SettlePG.js"></script>--%>
		<script type="text/javascript">
			...
			var totalAmount = '${order_info.fd_total_payment}';
			...
			function checkout_log() {
				let productInfoList = ${order_info_list_json};
			...
			$(document).ready(function (){
				...
				if (error) {
					...
					// }
					if (errorMessage !== '') {
						signInPopup.showMessage(errorMessage);
					} else if (code === "508") {
						signInPopup.showMessage('결제 진행 중 문제가 발생했습니다. 고객센터로 연락 부탁드립니다.');
					} else {
						signInPopup.showMessage('결제 진행중에 문제가 발생했습니다. 다시 시도해주세요.');
				...
				$("#selboxDirect").on("input", function () {
					...
					if (shipReq.length >= 30) {
						signInPopup.showMessage("최대 30자까지 입력가능합니다.");
						return;
					}
				});
					...
					<div class="real_area">
						...
						<div class="order_pd">
							...
							<div class="order_pd_detail">								
								<c:if test="${!empty order_info_list}">
								<div class="cart_pd normal_pd">
									...
									<div class="cart_pd_list normal_pd_list">
										<ul>
											<c:forEach var="item" items="${order_info_list}" varStatus="status">
											<li class="clear">
												<div class="img">
													<c:choose>
...
<%--																		<div class="cp_rdo">--%>
<%--&lt;%&ndash;																			<c:if test="${item.fd_coupon_type eq 'UC0003'}">&ndash;%&gt;--%>
<%--&lt;%&ndash;																				<input id="cpRdo${status.index}" type="radio" name="cp_rds_ship" value="${item.pk_idx}">&ndash;%&gt;--%>
<%--&lt;%&ndash;																				<label for="cpRdo${status.index}">배송비 할인</label>&ndash;%&gt;--%>
<%--&lt;%&ndash;																			</c:if>&ndash;%&gt;--%>
												...
												<li>
													<div class="pdc_li1 clear">
														...
														<div class="wrap clear">
															<p>원</p>
															<strong><fmt:formatNumber value="${order_info.fd_total_payment}" pattern="#,###" /></strong>
														</div>
													</div>
												</li>
<%--												<li class="clear">--%>
