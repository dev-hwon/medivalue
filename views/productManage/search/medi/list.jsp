	...
	<jsp:attribute name="javascript">
		...
		<script type="text/javascript">
			...
			$(document).ready(function (){
				...
				/* $("img.pd_thumb_img").each(function(){
					//console.log($(this)[0].naturalWidth + " | " + $(this)[0].naturalHeight);
					var obj = $(this);
					var naturalWidth = obj.naturalWidth; // img 너비
					var naturalHeight = obj.naturalHeight; // img 높이
					var imgWidth = $(obj).parent().parent().width(); // div 너비
					var imgHeight = $(obj).parent().parent().height(); // div 높이
				...
				$("button[id^='btn_buy']").on("click",function(e){
					var login_check = js_login_check();
					if(login_check){						
						let formData = {};
						let tpsm_idx = $(this).parent().parent().parent().children("input[name='tpsm_idx']").val();
						formData['list['+0+'].fk_tpsm_idx'] = tpsm_idx;
						formData['list['+0+'].cnt'] = 1;

						let pd_title = $(this).parent().parent().parent()
								.children(".info")
								.children(".info_wrap")
								.children(".pd_tit")
								.children("a")
							...
							url : '/basket/rest/save',
						 	...
						 	success:function(data){
						 		if(data.rtn_code != '200'){
									...
									ad_log_naver("3", "1");
									labbit_add_to_cart(data.productInfoList);
						 			setTimeout(function() { location.replace("/product/basket/list?pk_idx=" + tpsm_idx);}, 2000);
						 		}
						 		proc_state = true;
						 	},
						 	error:function(request,status,error){
				...
				$(document).on("click", ".btn_product_medicash_top_order, #buy_medicash", function () {
					let login_info = '${loginInfo}';
					if (login_info != '') {
						let tpsm_idx = $(this).closest("li").find("#tpsm_idx").val();
						let price = $(this).closest("li").find(".price_info")
								.children(".price").children("p").text().replaceAll("원", "");
						...
						if (tpsm_idx === '' || tpsm_idx == null) {
							...
							$("#pop_alarm_msg_2").text(price + " 메디캐쉬를 결제 하시겠습니까?");
				...
				$(".le_ok").on("click", function () {
					...
					if (JSON.stringify(formData) === '{}') {
						...
						$.ajax({
							...
							success: function (data) {
								if (data['rtn_code'] === '200') {
									let medicash_url = window.location.origin + '/order/info/medicash/' + data['fd_order_id']
								...
								<c:forEach var="p_item" items="${p_list}" varStatus="state" >
									...
									<c:if test="${!state.first}">
										...
										<li class="clear <c:if test="${p_item.fd_option_count > 0}">option </c:if> <c:if test="${!empty p_item.fd_product_insure_code}">insure</c:if>">
											...
											<div class="img<c:if test="${p_item.fd_event_yn eq 'Y'}"> payback</c:if><c:if test="${p_item.fd_stock_sell_yn eq 'N'}"> soldout</c:if><c:if test="${p_item.fd_custom_use_point_yn eq 'Y'}"> mileage</c:if>">
												<div class="icon_list">
												...
												<c:choose>
													<c:when test="${p_item.fk_cate_1_idx eq '213' && p_item.fk_cate_2_idx eq '214'}">
														<a class="btn_product_medicash_top_order" onclick="return false" href="" target="_self">
															<c:if test="${p_item.fd_img_path_thumbnail eq ''}">
																<img class="pd_thumb_img" style="object-fit: contain;" src="${p_item.fd_img_path}" border="0" alt="" onerror="this.src='/resources/images/dummy/250x321.png'" />
															</c:if>
															<c:if test="${p_item.fd_img_path_thumbnail ne ''}">
																<img class="pd_thumb_img" style="object-fit:contain;" src="${p_item.fd_img_path_thumbnail}" border="0" onerror="this.src='/resources/images/dummy/250x321.png'" alt="" />
															</c:if>
														</a>
													</c:when>
													<c:otherwise>
											...
											<div class="info">
												<div class="info_wrap">
													...
													<p class="standard"><c:if test="${p_item.fd_product_standard ne '' }">규격 ㅣ ${p_item.fd_product_standard}</c:if><c:if test="${p_item.fd_product_standard eq '' }">&nbsp;</c:if></p>
												...
												<c:choose>
													<c:when test="${p_item.fk_cate_1_idx eq '213' && p_item.fk_cate_2_idx eq '214'}">
														<div class="btn_wrap clear">
															<button id="buy_medicash" type="button" style="width: 100%">구매하기</button>
														</div>
													</c:when>
													<c:otherwise>
														<div class="btn_wrap clear">
															<button class="cart_btn" id="btn_cart" type="button">장바구니</button>
															<button class="buy_btn" id="btn_buy" type="button">구매하기</button>
															<span class="bar"></span>
														</div>
