									...
									<ul>
										...
										<c:if test="${!empty list}">
											<c:forEach var="item" items="${list}" varStatus="state">
												<li class="td pd_li_${item.pk_idx} <c:if test="${item.fd_option_count > 0}">option </c:if> <c:if test="${!empty item.fd_product_insure_code}">insure</c:if>">
													<div class="td_list clear">
														<input type="hidden" name="tpsm_idx" id="tpsm_idx" value="${item.fk_tpsm_idx}" />
														<input type="hidden" name="product_amount" id="product_amount" value="${item.shop_product_amount}" />
														...
														<div class="one_td3 info">
															<div class="info_box">
																<span>${item.cate_1_name} / ${item.cate_2_name}</span>
